...
@pytest.fixture
def delete_credit_card_view():
    account_model_mock = create_account_model_mock(exists=True)
    controller = DeleteCreditCardController(account_model_mock, create_card_model_mock(has_credit_card=True))
    view = DeleteCreditCardView(controller)
    return view


def test_create_credit_card_payment_view_success(delete_credit_card_view):
    ...
    with patch.object(delete_credit_card_view._DeleteCreditCardView__controller, 'operate') as mock_operate:
        ...
        response = delete_credit_card_view.handle(http_request)
...
def test_create_credit_card_payment_view_exception(delete_credit_card_view):
    valid_email = 'user@example.com'
    valid_token = token_creator.create(valid_email)
